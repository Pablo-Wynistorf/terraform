...
variable "addons-installed" {
  description = "Do you have AWS Cli, Helm, eksctl and kubectl installed? If not, do that first, otherwise the script won't work properly."
}
...
variable "var-nodecount" {
  description = "How many nodes do you have installed?"
}
...
resource "null_resource" "apply-kubectl-file" {
  ...
  provisioner "local-exec" {
    command     = "kubectl delete -f aws-load-balancer-controller-service-account.yaml"
    when        = destroy
  }
  depends_on = [ local_file.create-kubectl-file ]
}
...
resource "null_resource" "add-install-helm-repo" {
  provisioner "local-exec" {
    when    =  create
    command = "helm repo add eks https://aws.github.io/eks-charts && helm repo update && helm install aws-load-balancer-controller eks/aws-load-balancer-controller -n kube-system --set clusterName=${var.var-cluster-name} --set serviceAccount.create=false --set serviceAccount.name=aws-load-balancer-controller --set replicaCount=${var.var-nodecount}"
  }
  provisioner "local-exec" {
    when    =  destroy
    command = "helm uninstall aws-load-balancer-controller -n kube-system && helm repo remove eks"
  }
}
